...
import "../index.css";
import {deleteUser, getLevels, getUsers, patchLevelContent} from "../api/ServiceBus.js";
import {
    Button,
    MenuItem,
    Paper,
...
const Levels = () => {
    ...
    return (
        <div className="levels-block">
            ...
            <Select variant={"outlined"}  value={selectedLevel} onChange={handleLevelChange}>
                {levelData.map((level, index) => (
                    <MenuItem key={index} value={level.id}>{level.title}</MenuItem>
                ))}
            </Select>
            {selectedLevel && <LevelEditor levelId={selectedLevel}/>}

            <div>   
                {selectedLevel && <CreateQuestions levelId={selectedLevel}/> }
            </div>
        </div>
    );
};
